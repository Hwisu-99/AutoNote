...
def write_note(vault_path: str, summary: dict, title_slug: str, excalidraw_filename: str) -> str:
    ...
    # concepts/entities는 frontmatter에 더 이상 안 남긴다 - node_store(각 노드
    # 파일의 sources)가 유일한 소스다(paper_notes/graph_builder.py, node_store.py
    # 참고). concept_label_by_id는 아래 본문(레퍼런스 표의 "비고" 칸)에 여전히
    # 쓰인다 - frontmatter와 무관하게 이 논문 자신의 서사를 만드는 용도다.
    frontmatter = {
        "title": summary["title"],
